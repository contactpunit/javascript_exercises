...
function getUsers(users) {
    ...
    for (const line of lines) {
        if (line.trim()) {
            let [username, , , , displayName,] = line.split(':')
            if (!displayName.trim()) displayName = 'unknown'
            m.set(username.trim(), displayName.replace(/,+/, ' ').trim())
        }
    }
    return m
}

console.log(getUsers(pw1))
console.log(getUsers(pw2))
console.log(getUsers(pw3))
console.log(getUsers(pw4))